...
\begin{figure}[h]
	...
	\begin{tabularx}{25cm}{X  X  X  X}
		...
		\hline\hline
		testAssignCmodule&Assing a dummy ECore model file&Through an exception: \textit{EcoreFileNotFound.class}&Check whether the ECore model already exists and CModule is correctly assigned\\
		
		ActionNotExist (testProcessJsonFile)&JSON file with a US without \enquote{Action} entry&Through an exception:  \textit{ActionInJsonFileNotFound.class}&Check whether there is an entry \enquote{Action} in the related US in JSON file\\
